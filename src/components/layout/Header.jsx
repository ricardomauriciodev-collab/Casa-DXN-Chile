...
function PVProgressBar({ totalPV }) {
  ...
  return (
    <div className="flex items-center gap-2 flex-1 max-w-40 md:max-w-60 text-xs">
      <div className="flex-1 h-2.5 bg-white rounded-full overflow-hidden relative">
        <div
          className={`h-full rounded-full transition-all duration-300 ${barColor(pct)}`}
          style={{ width: `${pct}%` }}
        />
        <div className="absolute top-0 left-[30%] w-0.5 h-full bg-black/30 rounded" />
      </div>
