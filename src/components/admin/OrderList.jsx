        ...
        <div className="flex gap-2">
          {['pendiente', 'aprobado', 'todos'].map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`text-sm px-3 py-1 rounded font-medium cursor-pointer transition ${
            ...
            <div className="flex items-start justify-between">
                ...
                <ul className="mt-2 space-y-1">
                  {o.items.map((item, i) => (
                    <li key={i} className="text-sm flex justify-between">
                      ...
                      <span className="text-gray-500">${(item.price * item.quantity).toLocaleString('es-CL')}</span>
                    </li>
                  ))}
                </ul>
                <div className="mt-2 pt-2 border-t flex gap-4 text-sm font-semibold">
              ...
              {o.status === 'pendiente' && (
                <div className="flex gap-2 shrink-0 ml-4">
                  <Button variant="primary" onClick={() => handleApprove(o.id)}>APROBAR</Button>
                  <Button variant="danger" onClick={() => handleReject(o.id)}>RECHAZAR</Button>
                </div>
              )}
            </div>
          </div>
